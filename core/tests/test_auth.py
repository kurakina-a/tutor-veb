...
class AuthTests(TestCase):
    ...
    def test_4_login_student_success(self):
        ...
        user = User.objects.create_user(
            username="loginstudent", password="StudentPass123"
        )
        user.is_student = True
        user.save()
        response = self.client.post(
            "/login/", {"username": "loginstudent", "password": "StudentPass123"}
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, "/student/dashboard/")
    ...
    def test_5_login_teacher_success(self):
        user = User.objects.create_user(
            username="loginteacher", password="TeacherPass123"
        )
        user.is_teacher = True
        user.save()
        response = self.client.post(
            "/login/", {"username": "loginteacher", "password": "TeacherPass123"}
        )
        self.assertEqual(response.status_code, 302)
        self.assertRedirects(response, "/teacher/dashboard/")
    ...
    def test_6_login_wrong_password(self):
        ...
        response = self.client.post(
            "/login/", {"username": "wronguser", "password": "WrongPass456"}
        )
        self.assertEqual(response.status_code, 200)
