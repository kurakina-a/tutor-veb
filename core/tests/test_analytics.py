...
from ..models import Answer, Option, Question, Teacher, Test
...
class AnalyticsTests(TestCase):
    ...
    def setUp(self):
        self.teacher_user = User.objects.create_user(
            username="literature_teacher", password="pass123", is_teacher=True
        )
    ...
    def test_statistics_shows_author_question_mistakes(self):
        Answer.objects.create(
            student=self.student_anna,
            test=self.test,
            question=self.q1,
            selected_option=self.q1_wrong,
            is_correct=False,
        )
        Answer.objects.create(
            ...
            test=self.test,
            question=self.q1,
            selected_option=self.q1_correct,
            is_correct=True,
        )
    ...
    def test_statistics_shows_multiple_choice_mistakes(self):
        Answer.objects.create(
            ...
            test=self.test,
            question=self.q2,
            selected_option=self.q2_correct1,
            is_correct=False,
        )
        Answer.objects.create(
            student=self.student_peter,
        ...
        Answer.objects.create(
            ...
            test=self.test,
            question=self.q2,
            selected_option=self.q2_correct2,
            is_correct=True,
        )
        response = self.client.get(reverse("teacher_statistics"))
        stats = response.context["stats"]
        ...
        q2_stat = next(
            (q for q in test_stat["questions"] if q["id"] == self.q2.id), None
        )
    ...
    def test_statistics_identifies_problem_question(self):
        ...
        stats = response.context["stats"]
        test_stat = next((s for s in stats if s["title"] == self.test.title), None)
        max_error_question = max(
            test_stat["questions"], key=lambda x: x["error_percent"]
        )
