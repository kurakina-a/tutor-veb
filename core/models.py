...
class TestResult(models.Model):  # результат прохождения теста
    ...
    test = models.ForeignKey(
        Test, on_delete=models.CASCADE, related_name="student_results"
    )
    deadline = models.DateTimeField(null=True, blank=True, verbose_name="Дедлайн")
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default="assigned")
    score = models.PositiveIntegerField(null=True, blank=True, verbose_name="Баллы")
    started_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Начало прохождения"
    )
...
class TeacherStudent(models.Model):
    teacher = models.ForeignKey(
        Teacher, on_delete=models.CASCADE, related_name="my_students"
    )
    student = models.ForeignKey(
        Student, on_delete=models.CASCADE, related_name="my_teacher"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    ...
    class Meta:
        unique_together = [
            "teacher",
            "student",
        ]  # один ученик не может быть добавлен дважды
...
class Comment(models.Model):
    ...
    def __str__(self):
        return (
            f"Комментарий к {self.answer.question.text[:30]} от {self.teacher.username}"
        )
